...
if ($fglob)
{
	...
	foreach ($fglob as $file)
	{
		if (is_file($file))
		{
			...
			$k = !filesize($file) ? '' : ',"'. str_replace($disallow_character, $replace_character, $ff = file_get_contents($file)). '"';
			$l += mb_strlen(str_replace([PHP_EOL, ' ', '　'], '', $ff), 'UTF-8');
			echo 'l(', $i, $j, $k, '),';
			++$i;
		}
	}
