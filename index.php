...
?><!doctype html><html lang="<?=$lang?>"><head><meta charset=utf-8><title>localOutliner</title><meta name=viewport content="width=device-width,initial-scale=1"><style>*{box-sizing:border-box}body{background:#222;color:#ccc;margin-left:10%;margin-right:10%;margin-top:3em;margin-bottom:3em}header{display:flex;align-items:start;justify-content:space-between;transition:background.5s}header button{background-color:inherit;padding:1em}article{background:rgba(0,0,0,.1);border:0;padding:1em;font-size:x-large;white-space:pre-wrap}button,label{border:0;cursor:pointer;opacity:.35;padding:.5em;transition:opacity.5s}button:hover,label:hover{opacity:1}[contenteditable]:focus{outline:0}ol{counter-reset:i;list-style-type:none;padding:0}li{margin-bottom:1em;padding:1em 2.5em;position:relative;transition:background.5s}h1,h2{font-family:serif;padding-left:.5em;padding-right:.5em}h1,h2,article,aside{border-color:transparent;font-weight:unset;line-height:1.8em}h2:before{cursor:move;content:"<?=sprintf($title[1], $title[2])?>";counter-increment:i}input,.hide{display:none}.hide~label:before{font-size:x-large;content:"<?=$icon[3]?>"}label{padding:.7em;z-index:2}aside{transition:opacity.3s,width.5s,height.5s;width:0;height:0;padding:0;opacity:0;overflow-y:auto;white-space:pre-wrap;z-index:1}label[for=a]{background-color:lightskyblue}#aa{background-color:lightcyan;color:lightskyblue}label[for=b]{background-color:palevioletred}#ba{background-color:lavenderblush;color:palevioletred}label[for=c]{background-color:lightgreen}#ca{background-color:honeydew;color:lightgreen}label[for=d]{background-color:goldenrod}#da{background-color:lemonchiffon;color:goldenrod}li button{background-color:inherit;padding:1em;position:absolute;top:0;right:0}li label:before{font-size:x-large;content:"<?=$icon[4]?>";padding:.1em;position:absolute;top:1.7em;left:.5em;z-index:1}small{position:fixed;bottom:0;left:45%;margin:.5em}select{background-color:rgba(0,0,0,.1);color:inherit;border:none;position:fixed;top:0;left:45%;width:10em;padding:.5em;overflow:hidden;outline:none}option{color:dimgray}</style></head><body><script>const d=document,b=d.body,w=window,asd="aside",lbl="label",ipt="input",copyright=d.title;w.onload=()=>{<?php
if ($fglob)
{
