<?php
$title = ['タイトル', '第"%s"話　', 'counter(i)', '新規タイトル', 'リカバリー'];
...
if (filter_has_var(INPUT_GET, 'backup'))
{
	...
	shell_exec("tar -czf $filename --exclude='0' $title[0]");
...
if ($recover = filter_input(INPUT_GET, 'recover', FILTER_SANITIZE_NUMBER_INT))
{
	if (is_file($recover_gz = $title[0]. '/0/'. basename($recover). '.tar.gz'))
	{
		...
		shell_exec("tar -C ./ -xf $recover_gz");
	}
}
...
if ($del && is_dir($del)) shell_exec("rm -r ./$del");
...
{
	rsort($gzs);
	echo 'aside.appendChild(aslt=slt.cloneNode(true)),aslt.accessKey="r",aslt.id="recover",aslt.style.fontSize="small",aslt.style.marginLeft=".7em",aslt.style.padding=".6em",aslt.add(new Option("', $title[4], '")),aslt.firstChild.disabled=true,';
	foreach ($gzs as $gz)
	{
